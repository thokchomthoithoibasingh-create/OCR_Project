...
def _align_table_rows(row_groups):
    ...
    of being shoved left into columns 1,2,3... which is what caused
    misaligned trailing rows.
    """
    ...
    for r in row_groups:
        ...
        if len(r) == 1:
            ...
            for c in r:
                nearest = min(range(k), key=lambda idx: abs(bands[idx] - _x_left(c)))
                row_out[nearest] = c["text"]
        aligned.append(row_out)
    return aligned
